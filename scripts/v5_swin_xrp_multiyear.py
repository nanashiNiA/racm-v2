"""V5: swin_v25 XRP multi-year (2022, 2024, 2025)
==================================================
XRP funding +17.3%/yr (最高)
"""
...
print('='*70,flush=True)
print('  V5: swin_v25 XRP multi-year',flush=True)
print('='*70,flush=True)
...
def custom_load(data_dir, start_date, end_date, symbol="BTCUSDT", freq='30min'):
    print(f'    Loader: {ASSET}',flush=True)
    all_ohlc=[];all_ticker=[]
...
if len(results)>=2:
    cf=1.0
    for r in results.values():cf *= (1+r['net_return']/100)
    cagr=cf**(1/len(results))-1
    print(f'\n  {len(results)}-year compound: {(cf-1)*100:+.1f}%')
