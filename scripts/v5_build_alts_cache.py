"""V5: XRP/LINK/DOGE cache build (Binance API only)
====================================================
XRP funding +14.6%/yr, LINK +16.4%/yr, DOGE +13.7%/yr
"""
...
print('='*70,flush=True)
print('  V5: XRP/LINK/DOGE cache (Binance API)',flush=True)
print('='*70,flush=True)
...
for symbol in ['XRPUSDT','LINKUSDT','DOGEUSDT']:
    print(f'\n=== {symbol} ===',flush=True)

    print(f'Fetching {symbol} funding...',flush=True)
    funding=fetch_funding(symbol)
    print(f'  {len(funding)} records, mean: {funding["fundingRate"].mean()*3*365*100:+.1f}%/yr',flush=True)

    for year in range(2021,2026):
        cache_file=os.path.join(CACHE_DIR,f'{symbol}_{year}0101_{year}1231_1H.pkl')
        if os.path.exists(cache_file):
            print(f'  {year} exists',flush=True)
            continue

        print(f'  Fetching {symbol} klines {year}...',flush=True)
